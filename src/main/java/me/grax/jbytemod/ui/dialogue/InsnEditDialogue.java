...
public class InsnEditDialogue extends ClassDialogue {
    ...
    public static void createInsertInsnDialog(MethodNode mn, AbstractInsnNode ain, boolean after) {
        ...
        input.add(clazz);
        if (JOptionPane.showConfirmDialog(Main.getInstance().getJByteMod(), panel, "Insert " + (after ? "after" : "before"), 2) == JOptionPane.OK_OPTION) {
            try {
                ...
                if (!hasSettings(newnode) || new InsnEditDialogue(mn, newnode).open()) {
                    ...
                    Main.getInstance().getJByteMod().getCodeList().loadInstructions(mn);
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
            ...
            nameField.getDocument().addDocumentListener(new DocumentListener() {
                ...
                private void setToolTip() {
                    ...
                    if (ownerField != null && !ownerField.getText().isEmpty() && !nameField.getText().isEmpty()) {
                        ClassNode node = Main.getInstance().getJByteMod().getJarArchive().getClasses().get(ownerField.getText());
                        if (node == null) {
                            try {
                    ...
                    descComputeButton.addActionListener(e -> {
                        if (nameField != null && ownerField != null) {
                            ...
                            if (!mName.isEmpty() && !mOwner.isEmpty()) {
                                ClassNode node = Main.getInstance().getJByteMod().getJarArchive().getClasses().get(mOwner);
                                if (node == null) {
                                    try {
